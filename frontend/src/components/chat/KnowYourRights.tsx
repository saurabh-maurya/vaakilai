...
/**
 * Curated "Know Your Rights" reference. Embedded inside the AI chat surface —
 * each topic expands to plain-language steps and can hand the question straight
 * to the AI assistant via `onAsk` (falls back to a /chat deep-link when absent).
 */
export function KnowYourRights({ onAsk }: { onAsk?: (prompt: string) => void }) {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {filtered.map((topic) => {
          ...
          const askPrompt = `Tell me more about ${topic.title} in India`;
          return (
